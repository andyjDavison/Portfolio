...
import { classes2 } from "@/data/classesData";
import { colors } from "./Education";

export function ClassesCarouel2() {
    ...
    >
      <CarouselContent>
        {classes2.map((item) => (
          <CarouselItem
            key={item.name}
            className={`flex items-center justify-center hover:cursor-pointer basis-48/100 lg:basis-1/3`}
          >
            <span
              className={`flex flex-col ${colors[item.color as keyof typeof colors]} w-44 lg:w-125 rounded-4xl border-2 justify-center lg:px-2 lg:py-0.5`}
            >
              <p
